...
public class ExamErrorActivity extends Activity implements OnClickListener, OnItemClickListener {
	...
	private class QueryTask extends AsyncTask<Void, Void, ArrayList<CauseInfo>> {
		...
		protected ArrayList<CauseInfo> doInBackground(Void... params) {
			list = DBManager.getInstance(ExamErrorActivity.this).query(ExamErrorColumns.TABLE_NAME);
			return list;
		}
		...
		protected void onPostExecute(ArrayList<CauseInfo> list) {
			...
			int lastSelect = ConfigPreferences.getInstance(ExamErrorActivity.this).isLastExamError();
			i = lastSelect - 1;
	...
	public void onClick(View v) {
		...
		case R.id.back:
			...
			if (isClickSelectSubject) {
				i = ConfigPreferences.getInstance(this).isLastExamError() - 1;
				i -= 1;
				isClickSelectSubject = false;
			} else {
				i -= 1;
			}
			ConfigPreferences.getInstance(this).setLastExamError(i + 1);
		...
		case R.id.subject:
			listHasId = DBManager.getInstance(this).queryHasId(ExamErrorColumns.TABLE_NAME);
		...
		case R.id.collect:
			myData.setReply(0);
			DBManager.getInstance(this).insert(ExamErrorColumns.TABLE_NAME, myData);
	...
	public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
		...
		relFour.setBackgroundResource(R.color.select_default);
		i = position;
		ConfigPreferences.getInstance(this).setLastExamError(position + 1);
	...
	protected void onDestroy() {
		super.onDestroy();
		ConfigPreferences.getInstance(this).setLastExamError(1);
	}
}
